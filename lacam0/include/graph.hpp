...
struct Graph {
  ...
  // build from occupancy grid (row-major, 1=free, 0=blocked), size must be w*h
  Graph(int w, int h, const std::vector<int> &occupancy);
  // disable copying (would double-free Vertex pointers)
  Graph(const Graph &) = delete;
  Graph &operator=(const Graph &) = delete;
  // enable moving (transfer ownership of Vertex pointers)

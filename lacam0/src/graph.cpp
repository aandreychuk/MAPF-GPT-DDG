...
Graph::Graph(int w, int h, const std::vector<int> &occupancy)
    : V(), U(w * h, nullptr), width(w), height(h)
{
  if ((int)occupancy.size() != w * h) return;
  // create vertices for free cells
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      const int index = w * y + x;
      if (occupancy[index] == 1) continue;
      auto v = new Vertex((int)V.size(), index, x, y);
      V.push_back(v);
      U[index] = v;
    }
  }
  // create edges and actions
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      auto v = U[w * y + x];
      ...
      height(other.height)
{
  other.width = 0;
  other.height = 0;
  // leave other's V with moved pointers; ensure other won't delete them
  other.V.clear();
  other.U.clear();
}

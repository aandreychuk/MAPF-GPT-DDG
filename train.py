...
wandb_run_path = None  # "entity/project/run_id"
wandb_run_name = None  # name of the run to resume
...
if ddp:
    init_process_group(backend=backend, timeout=datetime.timedelta(hours=2))
...
if init_from == "scratch":
    ...
    if checkpoint_files:
        ...
        ckpt_path = os.path.join(out_dir, f"ckpt_{dagger_type}.pt")
    ...
    logger.info(f"Resuming training from {iter_num}")
...
if wandb_log and master_process:
    ...
    try:
        api = wandb.Api()
        previous_run = api.run(wandb_run_path)
        wandb.init(
        ...
        logger.error(f"Failed to resume wandb run: {e}")
